          ...
          <div  v-for="(mydata,index) in alldata.list" :key="index">
            ...
            <div v-else="" >
              ...
              <div class="community_detail01"> <div class="thumb-a"> <label class="dianzanlabel" > <input  @click="bad(mydata.rid,index)" :disabled="mydata.isevaluated===1?'disabled':false" :checked="mydata.isevaluated===2?'checked':false" type="checkbox" class="mycheckbox" id="'+id2+'" hidden> <svg  t="1648040495290" class="icon dianzansvg" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5400" height="50" width="300"><path class="heartclass" d="M320 189.6v343.2c0 10.4 5.6 20 14.4 25.6 80 48 220.8 168.8 220.8 271.2 0 4.8-5.6 66.4 40.8 66.4 0 0 118.4-28 44.8-320h208s48 4 48-48.8c0 0 2.4-66.4-68-324.8 0 0-21.6-74.4-48.8-74.4 0 0-340 32-430.4 32-16.8 0-29.6 12.8-29.6 29.6zM146.4 155.2l96 4c8 1.6 13.6 8 13.6 16v355.2c0 8-5.6 14.4-13.6 16l-96 3.2c-9.6 1.6-18.4-5.6-18.4-16V171.2c0-10.4 8.8-17.6 18.4-16z" p-id="5401"></path></svg> <span class="thumbclass"></span></label></div>
                <div class="thumb-b"> <label class="dianzanlabel" > <input @click="canshow ?good(mydata.rid,index) : mygood(mydata.rid,index)"  :disabled="mydata.isevaluated===2?'disabled':false" :checked="mydata.isevaluated===1?'checked':false" type="checkbox"  class="mycheckbox" hidden> <svg t="1639041971928" class="icon dianzansvg" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3128"><path  class="heartclass" d="M512 896a42.666667 42.666667 0 0 1-30.293333-12.373333l-331.52-331.946667a224.426667 224.426667 0 0 1 0-315.733333 223.573333 223.573333 0 0 1 315.733333 0L512 282.026667l46.08-46.08a223.573333 223.573333 0 0 1 315.733333 0 224.426667 224.426667 0 0 1 0 315.733333l-331.52 331.946667A42.666667 42.666667 0 0 1 512 896z" p-id="3129" ></path></svg> <span class="thumbclass"></span> </label><div class="community_detail02"><span >{{mydata.rrank}}</span></div> </div> </div>
            </div>
            <hr>
          </div>
        </div>
      <div v-if="canshow" class="modal-body" id="community_detail06">
        <div id="aadiv">
          <div v-if="alldata.myanswer.rid===-1">
            <div v-show="this.show" id="myans" class="myans">
              <textarea id="inputans" class="form-control" rows="5" placeholder="请输入您的回答" v-model="newcontent"></textarea>
              <button type="button" class="btn btn-info" @click="newreply(alldata.mid)">提交</button>
            </div>
            <button @click="toreply" type="button" class="btn btn-primary">我来答</button>
          </div>
          <div v-else="">
            <div>
              <img v-if="alldata.myanswer.user.address=='none'" src="@/assets/img/img.png" class="img-circle" width="30" height="30" alt="头像"/>
              <img v-else :src='"http://localhost:80/api/gg/"+alldata.myanswer.user.address' class="img-circle" width="30" height="30" alt="头像"/>
              <span>{{alldata.myanswer.user.name}}   </span>
              <span>{{alldata.myanswer.rtime}}</span>
            </div>
            <div>
              <p class="fs-4" id="community_detail_content02">{{alldata.myanswer.rcontent}}</p>
              <div id="community_detail03">
                <button type="button" @click="reviseans" class="btn btn-default">修改回答</button>
                <button @click="deleteans" type="button" class=" btn btn-default">删除回答</button></div>
              <div id="community_detail04">
                <div class="thumb-a"> <label class="dianzanlabel" > <input  @click="bad(alldata.myanswer.rid,-1)" type="checkbox" class="mycheckbox" :disabled="alldata.myanswer.isevaluated==1?'disabled':false" :checked="alldata.myanswer.isevaluated===2?'checked':false"  hidden> <svg  t="1648040495290" class="icon dianzansvg" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5400" height="50" width="300"><path class="heartclass" d="M320 189.6v343.2c0 10.4 5.6 20 14.4 25.6 80 48 220.8 168.8 220.8 271.2 0 4.8-5.6 66.4 40.8 66.4 0 0 118.4-28 44.8-320h208s48 4 48-48.8c0 0 2.4-66.4-68-324.8 0 0-21.6-74.4-48.8-74.4 0 0-340 32-430.4 32-16.8 0-29.6 12.8-29.6 29.6zM146.4 155.2l96 4c8 1.6 13.6 8 13.6 16v355.2c0 8-5.6 14.4-13.6 16l-96 3.2c-9.6 1.6-18.4-5.6-18.4-16V171.2c0-10.4 8.8-17.6 18.4-16z" p-id="5401"></path></svg> <span class="thumbclass"></span></label></div>
                <div class="thumb-b"> <label class="dianzanlabel" > <input @click="good(alldata.myanswer.rid,-1)"  type="checkbox"  :disabled="alldata.myanswer.isevaluated==2?'disabled':false" class="mycheckbox"  :checked="alldata.myanswer.isevaluated===1?'checked':false" hidden> <svg t="1639041971928" class="icon dianzansvg" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3128"><path  class="heartclass" d="M512 896a42.666667 42.666667 0 0 1-30.293333-12.373333l-331.52-331.946667a224.426667 224.426667 0 0 1 0-315.733333 223.573333 223.573333 0 0 1 315.733333 0L512 282.026667l46.08-46.08a223.573333 223.573333 0 0 1 315.733333 0 224.426667 224.426667 0 0 1 0 315.733333l-331.52 331.946667A42.666667 42.666667 0 0 1 512 896z" p-id="3129" ></path></svg> <span class="thumbclass"></span> </label><div id="community_detail05"> <span >{{alldata.myanswer.rrank}}</span> </div></div> </div>
            </div>
            <div v-show="this.show" id="reviseans" class="myans">
              <textarea v-model="newcontent" class="form-control" rows="5" placeholder="请输入您的回答"></textarea>
              <button type="button" class="btn btn-default" @click="cancle()">取消</button>
              <button @click="tosubmit()" type="button" class="btn btn-primary">提交修改</button>
            </div>
          </div>
          <hr/>
          <div>
...
export default {
  ...
  methods:{
    ...
    mygood(rid,i)
    {
      axios.post(this.server+'/historygood',{
        rid
      }).then(
          response=>
          {
